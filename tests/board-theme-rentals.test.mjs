...
test("eleven rentable themes appear in the catalog and migration", () => {
  const entries = [...catalog.matchAll(/\{ id: "([a-z-]+)", name: "([^"]+)", group: "(included|theme|color)"/g)];
  assert.equal(entries.filter(entry => entry[3] === "theme").length, 6);
...
test("expired rentals fall back to included colors and purchases are idempotent", () => {
  ...
  assert.match(api, /active=\['slate','classic','wood','meta-blue'\]/);
