...
test('Offline replay and OCBR sync are idempotent, separate from CBR, and only affect the submitting account',async()=>{
 ...
 await privateAction(db,b,'offline-result',{record});await privateAction(db,b,'offline-result',{record});assert.equal(player(db,'b').cbr,88);assert.equal(player(db,'b').ocbr,94);assert.equal(player(db,'a').cbr,88);assert.equal((await privateAction(db,b,'offline-history',{})).games.length,1);await privateAction(db,a,'offline-result',{record});assert.equal(player(db,'a').cbr,88);assert.equal(player(db,'a').ocbr,79);
