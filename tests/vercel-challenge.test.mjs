...
async function request(publicChallenge, rejectFeed = false) {
  ...
  try {
    await handler({ method: 'POST', headers: { authorization: 'Bearer test-user-token' }, body: { action: 'room', control: '10+0', publicChallenge } }, res);
    return { ...res, writes };
  } finally {
