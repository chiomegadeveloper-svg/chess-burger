...
test("Play shortcuts open destination selection and both destinations are active",()=>{assert.match(page,/key === "play" \? "play-select" : key/);assert.match(page,/finishWelcome\("play-select"\)/);assert.match(page,/className="play-destination arena arena-live"[^>]*onClick=\{\(\) => setTab\("tournaments"\)\}/);assert.doesNotMatch(page,/Grand Arena is coming soon/);assert.match(css,/\.play-destination\.arena-live:after\{content:"LIVE"/);});
