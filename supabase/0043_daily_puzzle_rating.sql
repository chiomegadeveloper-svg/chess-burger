...
create table if not exists public.cb_puzzle_profiles(
  user_id uuid primary key references public.cb_profiles(user_id) on delete cascade,
  puzzle_rating integer not null default 1000 check(puzzle_rating between 100 and 4000),
  solved_total integer not null default 0 check(solved_total>=0),
  correct_streak integer not null default 0 check(correct_streak>=0),
  best_streak integer not null default 0 check(best_streak>=0),
  last_solved_day date,
  updated_at timestamptz not null default now()
);
alter table public.cb_puzzle_profiles enable row level security;
revoke all on public.cb_puzzle_profiles from public,anon,authenticated;

create or replace function public.cb_claim_daily_puzzle(p_user_id uuid,p_day date,p_puzzle_id text,p_is_final boolean,p_puzzle_rating integer)
...
begin
  ...
  if v_inserted is not null then
    ...
    if p_is_final and v_count=3 then
...
revoke all on function public.cb_claim_daily_puzzle(uuid,date,text,boolean,integer) from public,anon,authenticated;
grant execute on function public.cb_claim_daily_puzzle(uuid,date,text,boolean,integer) to service_role;
