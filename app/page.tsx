...
export default function Page(){
 ...
 const showOnlineResult=useCallback((m:ArenaMatch)=>{const ownId=profile?.user_id;if(!ownId||m.status!=='finished'||![m.white_id,m.black_id].includes(ownId))return;const key=ownId+':'+m.id,storageKey='cb-shown-match-results:'+ownId;let saved:string[]=[];try{saved=JSON.parse(localStorage.getItem(storageKey)??'[]');}catch{}if(shownResults.current.has(key)||saved.includes(m.id))return;const side=m.white_id===ownId?'white':'black',matchPlayer=side==='white'?m.white:m.black,opponent=side==='white'?m.black:m.white;const player=matchPlayer??{user_id:ownId,display_name:profile.display_name,username:profile.username,avatar_url:profile.avatar_url,cbr:profile.cbr,wins:profile.wins,losses:profile.losses,win_streak:profile.win_streak};shownResults.current.add(key);try{localStorage.setItem(storageKey,JSON.stringify([...saved.filter(id=>id!==m.id),m.id].slice(-50)));}catch{}setSummary({id:m.id,outcome:m.result==='draw'?'draw':m.result===side?'win':'loss',player,delta:m.rating_changes?.[ownId]??(player.cbr-(side==='white'?m.white_cbr:m.black_cbr)),opponent});},[profile]);
 const [localOcbr,setLocalOcbr]=useState(88);
 const scroller=useRef<HTMLDivElement>(null),gps=useGpsPresence(profile?.user_id);
 const refreshProfile=useCallback(async()=>{try{const c=await getSupabase();if(!c)return;const {data:{session}}=await c.auth.getSession();if(!session)return;const data=await profileRequest(c);if(!data){setMember(false);setTab('profile');return;}setMember(true);let current=data;try{const live=await arena<{profile:PlayerProfile}>('me');current=live.profile;}catch{}const stored=localStorage.getItem('cb-local-ocbr:'+current.user_id);if(stored)current={...current,ocbr:Math.max(0,Number(stored)||88)};setProfile(current);setLocalOcbr(current.ocbr??88);authStorage.setItem('cb-staff-profile',JSON.stringify(current));}catch{}},[]);
 const openMatch=(id:string)=>{setMatchId(id);setActiveId(id);setTab('game');};
 const onSaved=(p:PlayerProfile)=>{setMember(true);setProfile(p);setLocalOcbr(p.ocbr??88);window.dispatchEvent(new Event('cb-profile-saved'));void refreshProfile();setTab('home');};
 function rateLocal(result:'win'|'loss'|'draw',gameId:string){
  ...
  const delta=result==='win'?6:result==='loss'?-Math.min(9,localOcbr):0,next=Math.max(0,localOcbr+delta);setLocalOcbr(next);localStorage.setItem('cb-local-ocbr:'+String(profile?.user_id??'guest-device'),String(next));localStorage.setItem(ledgerKey,JSON.stringify([...ledger,gameId]));
  if(profile){const updated={...profile,ocbr:next};setProfile(updated);if(profile.user_id==='guest-device')localStorage.setItem('cb-guest-profile',JSON.stringify(updated));else authStorage.setItem('cb-staff-profile',JSON.stringify(updated));}
  toast.success('Offline OCBR '+(delta>=0?'+':'')+delta);return delta;
 }
 function localResult(m:ArenaMatch,id:string){const side=id===m.white_id?'white':'black',result=m.result==='draw'?'draw':m.result===side?'win':'loss';const delta=rateLocal(result,m.id);if(delta===null)return;setSummary({id:m.id,outcome:result,delta,player:profile??{cbr:88,wins:0,losses:0,win_streak:0},ratingLabel:'OCBR',ratingValue:Math.max(0,localOcbr+delta),opponent:side==='white'?m.black:m.white,local:true});saveGame({id:m.id,white:m.white?.display_name??'White',black:m.black?.display_name??'Black',pgn:m.pgn,score:m.result==='draw'?'½–½':m.result==='white'?'1–0':'0–1',startedAt:new Date(m.created_at).toISOString(),updatedAt:new Date().toISOString(),ratedAt:new Date().toISOString(),ratingDelta:delta});}
 useEffect(()=>{
  ...
  try{const raw=!navigator.onLine?authStorage.getItem('cb-staff-profile')??localStorage.getItem('cb-guest-profile'):localStorage.getItem('cb-guest-profile');if(raw){const cached=JSON.parse(raw);setProfile(cached);const rating=localStorage.getItem('cb-local-ocbr:'+String(cached.user_id??'guest-device'));setLocalOcbr(rating?Math.max(0,Number(rating)||88):cached.ocbr??88);}}catch{}
  ...
  const clear=()=>{setMember(false);setProfile(null);setSummary(null);setActiveId('');setInvites([]);setLocalOcbr(88);setTab('profile');};window.addEventListener('cb-signed-out',clear);
 ...
 useEffect(()=>{if(!profile||profile.user_id==='guest-device')return;let live=true,pending=false;const poll=async()=>{if(pending||!navigator.onLine)return;pending=true;try{const d=await arena<{invites:Invite[];match:ArenaMatch|null}>('state');if(live){setInvites(d.invites);setActiveId(d.match?.status==='active'?d.match.id:'');}}catch{}finally{pending=false;}};void poll();const timer=setInterval(poll,4000);return()=>{live=false;clearInterval(timer);};},[profile?.user_id]);
