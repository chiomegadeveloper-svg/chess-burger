...
export default function Page(){
 ...
 const showOnlineResult=useCallback((m:ArenaMatch)=>{const ownId=profile?.user_id;if(!ownId||m.status!=='finished'||![m.white_id,m.black_id].includes(ownId))return;const key=ownId+':'+m.id;if(shownResults.current.has(key))return;const side=m.white_id===ownId?'white':'black',player=side==='white'?m.white:m.black;if(!player)return;shownResults.current.add(key);setSummary({id:m.id,outcome:m.result==='draw'?'draw':m.result===side?'win':'loss',player,delta:m.rating_changes?.[ownId]??0,opponent:side==='white'?m.black:m.white});},[profile?.user_id]);
