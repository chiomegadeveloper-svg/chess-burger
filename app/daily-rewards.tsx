...
const rewardIcon=(reward:Reward)=>reward.kind==="gold"?"/currency/cbg-coin.webp":reward.kind==="arena_ticket"?"/grand-arena/arena-ticket.webp":reward.kind==="bag_slot"?"/daily-rewards/bag-slot.webp":"/daily-rewards/day-2.webp";
const rewardLabel=(reward:Reward)=>reward.kind==="gold"?`${reward.amount} CBG`:reward.kind==="arena_ticket"?`${reward.amount} Arena Ticket${reward.amount===1?'':'s'}`:reward.kind==="bag_slot"?`${reward.amount} Bag Slot${reward.amount===1?'':'s'}`:`${reward.name??"Feed Banner"} · ${reward.days??reward.amount} days`;
const rewardDetails=(reward:Reward)=>reward.kind==="gold"?`${reward.amount} CBG coins`:reward.kind==="arena_ticket"?`${reward.amount} Arena Ticket${reward.amount===1?'':'s'}`:reward.kind==="bag_slot"?`${reward.amount} additional Bag Slot${reward.amount===1?'':'s'}`:`${reward.name??"Feed Banner"}, active for ${reward.days??reward.amount} days`;
...
export default function DailyRewards(){
  ...
  return <section className="daily-reward-page" aria-labelledby="daily-reward-title"><div className="daily-reward-modal">
    ...
    <div className="daily-reward-grid">{shown.rewards.map(reward=>{const current=reward.day===shown.day,done=reward.day<shown.day||shown.claimed_today&&current,details=`Day ${reward.day}: ${rewardDetails(reward)}`;return <article key={reward.day} tabIndex={0} role="button" aria-expanded={detailsDay===reward.day} aria-describedby={`reward-detail-${reward.day}`} aria-label={`${details}. Tap to ${detailsDay===reward.day?"hide":"show"} details.`} onClick={()=>setDetailsDay(day=>day===reward.day?null:reward.day)} onKeyDown={event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();setDetailsDay(day=>day===reward.day?null:reward.day);}}} className={`daily-reward-card ${reward.kind} day-${reward.day}${current?" current":""}${done?" claimed":""}${detailsDay===reward.day?" details-open":""}`}>
      <div className="daily-reward-day">DAY {reward.day}</div><div className="daily-reward-art"><img className="daily-reward-icon" width={128} height={128} src={rewardIcon(reward)} alt={details}/><strong>{reward.kind==="banner"?`${reward.days??reward.amount} DAYS`:reward.amount}</strong><small>{reward.kind==="gold"?"CBG":reward.kind==="arena_ticket"?"ARENA TICKET":reward.kind==="bag_slot"?"BAG SLOT":reward.name??"FEED BANNER"}</small></div><div className="daily-reward-tooltip" id={`reward-detail-${reward.day}`} role="tooltip"><b>{rewardDetails(reward)}</b><span>{reward.kind==="gold"?"Added instantly to your CBG balance.":reward.kind==="arena_ticket"?"Stored in your Bag for Grand Arena entry.":reward.kind==="bag_slot"?"Permanently expands your Bag capacity.":`${reward.name??"Feed Banner"} is added to your Bag as a timed rental.`}</span></div>
