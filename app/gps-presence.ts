...
export function useGpsPresence(userId?:string){
 const [enabled,setEnabled]=useState(false),[position,setPosition]=useState<Position|null>(null),[error,setError]=useState('');const latest=useRef<Position|null>(null),territory=useRef<TerritoryLabel|undefined>(),lastLocated=useRef(0);
