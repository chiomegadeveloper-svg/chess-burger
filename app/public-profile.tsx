...
import {ArrowLeft,Trophy,Coins,Crown,Swords} from "lucide-react";
...
export default function PublicProfile({userId,onClose,currentUserId,onChallenge}:{userId:string;onClose:()=>void;currentUserId?:string;onChallenge:(player:ArenaPlayer)=>void}){
 ...
 return <section className="public-profile"><button className="back-button" onClick={onClose}><ArrowLeft/>Community feed</button><div className="public-profile-hero cloud-panel"><span className="public-avatar">{profile.avatar_url?<img src={profile.avatar_url} alt={profile.display_name}/>:profile.display_name.charAt(0)}</span><div><p>@{profile.username}</p><h1>{profile.display_name}</h1><span>{profile.country_code} · Level {level.level} · {level.name}</span></div><img className="public-level" src={`/levels/level-${String(level.level-1).padStart(2,"0")}.png`} alt={level.name}/></div>{profile.user_id!==currentUserId&&<div className="profile-connect cloud-panel"><h2>Connect with {profile.display_name}</h2><button className="gold-button" onClick={()=>onChallenge(profile)}><Swords size={16}/> Challenge</button><SocialButtons target={profile.user_id} onBlocked={()=>setError("You blocked this player. Manage blocked players in Friends.")}/></div>}<div className="public-stats"><article><Trophy/><strong>#{rank}</strong><span>Current rank</span></article><article><Crown/><strong>{profile.cbr}</strong><span>CBR</span></article><article><Coins/><strong>{profile.gold_points}</strong><span>Gold</span></article><article><strong>{rate}%</strong><span>Win rate</span></article></div>{profile.bio&&<div className="cloud-panel public-bio"><h2>About</h2><p>{profile.bio}</p></div>}<div className="cloud-panel public-record"><h2>Player record</h2><span>{profile.wins} wins</span><span>{profile.losses} losses</span><span>{profile.win_streak} win streak</span></div><ProfilePhotoBucket photos={profile.featured_photos} name={profile.display_name}/><Testimonials profileId={profile.user_id} currentUserId={currentUserId}/></section>;
}
