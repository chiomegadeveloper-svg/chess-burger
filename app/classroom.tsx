...
export default function Classroom({onBack,onOpenShop}:{onBack:()=>void;onOpenShop:()=>void}){
  ...
  const quit=async(room:Room)=>{if(!window.confirm(`Quit ${room.name}? You will leave this classroom and need its room code to join again.`))return;setBusy(true);try{await classroom("quit",{room_id:room.id});toast.success(`You left ${room.name}.`);await load()}catch(e){toast.error(e instanceof Error?e.message:"Unable to quit this classroom.")}finally{setBusy(false)}};
  ...
  return <section className="classroom-page">
    ...
    {!role?<div className="classroom-role-grid">
      ...
      {role==="teacher"?<div className="classroom-columns">
        ...
        <section className="classroom-panel"><div className="panel-title"><BookOpen/><span><small>CLASSROOM SESSION TAB</small><h2>Your active rooms</h2></span></div><div className="session-list">{state?.owned?.length?state.owned.map(r=><article key={r.id}><div><small>{r.package_kind.toUpperCase()}</small><h3>{r.name}</h3><span>{r.student_count??0}/{r.max_students} students · package added {r.cbc_included??0} CBC to your Bag · ends {new Date(r.expires_at).toLocaleString()}</span></div><button className="enter-workshop" onClick={()=>setWorkshop(r.id)}><GraduationCap size={14}/>Enter Workshop</button><button onClick={()=>navigator.clipboard.writeText(r.invite_code).then(()=>toast.success("Code copied"))}><KeyRound size={14}/>{r.invite_code}<Copy size={12}/></button><button onClick={()=>void rename(r)}>Rename</button></article>):<p className="empty-room">No active Room Sessions yet.</p>}</div></section>
