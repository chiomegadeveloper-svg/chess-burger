  ...
  return <section className="classroom-page">
    ...
    {!role?<div className="classroom-role-grid">
      ...
      {role==="teacher"?<div className="classroom-columns">
        ...
        <section className="classroom-panel join-panel"><div className="panel-title"><KeyRound/><span><small>PRIVATE CLASS CODE</small><h2>Join a Room Session</h2></span></div><div className="student-credit-status"><img src="/classroom/cbc-token.webp" alt="CBC"/><span><small>AVAILABLE TO ENROLL</small><strong>{state?.wallet?.cbc??0} CBC</strong></span><button type="button" onClick={onOpenShop}><ShoppingBag size={14}/>Buy CBC</button></div><p>First enrollment uses 1 CBC from My Bag. Your teacher can also gift CBC to your username. Returning to the same room is free.</p><label>8-character classroom code<input maxLength={8} value={code} onChange={e=>setCode(e.target.value.replace(/[^a-z0-9]/gi,"").toUpperCase())} placeholder="PAWN1234" autoCapitalize="characters"/></label><button className="classroom-primary" disabled={busy||code.length!==8||(state?.wallet?.cbc??0)<1} onClick={()=>void join()}>{busy?"Checking classroom…":(state?.wallet?.cbc??0)<1?"Buy or receive CBC to enroll":"Join Room Session · 1 CBC"}</button></section>
