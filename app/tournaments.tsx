...
function readLocal(): Tournament[] {
  try {
    return JSON.parse(localStorage.getItem(KEY) ?? "[]").map(parseTournament);
  } catch {
    return [];
  }
}
function saveLocal(t: Tournament) {
  const all = readLocal();
  localStorage.setItem(
    KEY,
    JSON.stringify([t, ...all.filter((x) => x.id !== t.id)]),
  );
}
function audit(t: Tournament, action: string) {
  const key = "cb-offline-audit";
...
export default function Tournaments({
  ...
  useEffect(() => {
    setAll(readLocal());
    let live = true;
    if (host && profile && navigator.onLine)
      void getSupabase()
        .then(async (c) => {
          if (!c) return;
          ...
          const remote = (data ?? []).map(
            (row: { state: unknown; revision: number }) => ({
              ...parseTournament(row.state),
          ...
          const merged = [
            ...local,
            ...remote.filter(
              (r: Tournament) => !local.some((l) => l.id === r.id),
            ),
          ];
  ...
  const receiveInvite = useCallback((raw: string) => {
    try {
      ...
      if (raw.includes("#tournament=")) {
        data = JSON.parse(decodeURIComponent(raw.split("#tournament=")[1]));
      } else data = JSON.parse(raw);
      const d = data as Invite & { type: string };
      if (
        d.type !== "cb-tournament" ||
        !d.id?.match(/^[0-9a-f-]{36}$/i) ||
        !d.code?.match(/^[A-Z0-9]{8}$/) ||
      ...
      setInvite(d);
      setNotice(
        "Invitation loaded. Register online or show the host your registration QR.",
      );
    } catch {
      toast.error("This is not a Chess Burger tournament invitation.");
    }
  }, []);
  useEffect(() => {
    const readInvite = () => {
  ...
  async function sync(t: Tournament) {
    const c = await getSupabase();
    if (!c) throw Error("Connect your account to publish this tournament.");
    const { data, error } = await c.rpc("cb_save_tournament", {
      p_id: t.id,
      p_code: t.code,
      p_title: t.title,
      p_state: t,
      p_revision: t.revision ?? 0,
    });
    if (error) throw Error(error.message);
    const saved = { ...t, revision: Number(data) };
    saveLocal(saved);
    setEvent(saved);
    setAll(readLocal());
    return saved;
  ...
  function addNames() {
    ...
    for (const name of incoming) {
      if (name.length > 60) {
        setNotice("Player names must be 60 characters or fewer.");
        return;
      }
      if (!players.some((p) => p.name.toLowerCase() === name.toLowerCase()))
        players.push({ id: players.length + 1, name, rating: 0 });
    }
    if (players.length > 100) {
      setNotice("Maximum 100 players per event.");
      return;
    }
    persist({ ...event, players }, "Added players");
    setNames("");
  }
  const acceptQr = useCallback(
    (raw: string) => {
      try {
        if (!event || !editable || event.status !== "registration")
          throw Error("Registration is closed.");
        const d = JSON.parse(
          raw.includes("#registration=")
            ? decodeURIComponent(raw.split("#registration=")[1])
            : raw,
        );
        if (
          d.type !== "cb-registration" ||
          d.eventId !== event.id ||
          d.code !== event.code ||
          ...
          throw Error("Registration QR does not match this tournament.");
        if (
          event.players.some(
        ...
        if (event.players.length >= 100) throw Error("The event is full.");
        persist(
          {
            ...event,
            players: [
              ...event.players,
              { id: event.players.length + 1, name: d.name.trim(), rating: 0 },
            ],
          },
          "Accepted QR registration",
        );
        toast.success(
          "Player added. Offline identity is confirmed by the host.",
        );
      } catch (e) {
  ...
  async function nextRound() {
    ...
    if (event.mode === "offline") {
      ...
      if (!token)
        throw Error(
          "Sign in and connect to the internet to use the Swiss API.",
        );
      const r = await fetch("/api/swiss", {
        method: "POST",
        headers: {
      ...
      if (!r.ok) throw Error(data.error ?? "Pairing service is unavailable.");
      if (
  ...
  function result(board: number, value: Result) {
    ...
    const history = event.history.map((r) => r.map((g) => ({ ...g })));
    history[history.length - 1][board].result = value;
    persist(
      { ...event, history },
      "Recorded round " + history.length + " board " + (board + 1),
    );
  }
  async function loadRegistrations() {
    if (!event) return;
    const c = await getSupabase();
    if (!c) throw Error("Connect to load registrations.");
    const { data, error } = await c
      .from("cb_tournament_entries")
  ...
  function acceptOnline() {
    ...
    for (const p of registrations)
      if (
        !players.some(
          (x) =>
            x.userId === p.user_id ||
            x.name.toLowerCase() === p.display_name.toLowerCase(),
        )
      )
        players.push({
          id: players.length + 1,
    ...
    if (players.length > 100) {
      setNotice("The event exceeds 100 players.");
      return;
    }
    persist({ ...event, players }, "Accepted online registrations");
    setRegistrations([]);
  }
  async function register() {
    if (!invite) return;
    const c = await getSupabase();
    if (!c) throw Error("Sign in or use the offline registration QR.");
    const { error } = await c.rpc("cb_join_tournament", {
      p_id: invite.id,
      p_code: invite.code,
    });
    if (error) throw Error(error.message);
  ...
  async function refreshOnline() {
    if (!event) return;
    const c = await getSupabase();
    if (!c) throw Error("Connect to load the saved event.");
    const { data, error } = await c
      .from("cb_tournaments")
      .select("state,revision")
      .eq("id", event.id)
      .single();
    if (error) throw Error(error.message);
    const next = { ...parseTournament(data.state), revision: data.revision };
    saveLocal(next);
    setEvent(next);
    setAll(readLocal());
    setNotice("Loaded the published version.");
  }
  async function awardGold() {
    if (
      ...
      return;
    const podium = standings(event).slice(0, 3);
    if (podium.length < 3 || podium.some((player) => !player.userId))
      throw Error(
    ...
    await arena("tournament-gold", {
      ...
      winners: podium.map((player, index) => ({
        ...
        amount: [rewards.champion, rewards.second, rewards.third][index],
      })),
    });
    const next = persist(
    ...
    setNotice("Tournament Gold was awarded and logged.");
  }
  const qr = event
    ? JSON.stringify({
        type: "cb-tournament",
      ...
      {staff && !event && (
        <>
          ...
          <label>
            ...
            >
              <option value="offline">Offline club Swiss</option>
              ...
              onChange={(e) => {
                ...
                if (file)
                  void perform(async () => {
                    ...
                    const t = parseTournament(JSON.parse(await file.text()));
                    if (t.hostId !== profile?.user_id)
                      throw Error(
                        "This backup belongs to another host account.",
                      );
                    persist(t, "Restored backup");
                  });
              }}
            />
          </label></section>
        ...
        <>
          <div className="cms-actions tournament-cms-toolbar">
            ...
            <button
              ...
              onClick={() =>
                void perform(async () => {
                  await sync(event);
                  setNotice("Tournament published and activity logged.");
                })
              }
            >
              Publish / sync
            ...
            <button
              disabled={busy || !event.revision}
              onClick={() => void perform(refreshOnline)}
            >
          ...
          {editable && event.status === "registration" && (
            <section className="tournament-cms-section"><div className="tournament-cms-section-head"><span>02 · REGISTRATION</span><h3>Prizes and players</h3></div>
              ...
              <button onClick={addNames}>Add players</button>
              <QrInput label="Scan player registration" onValue={acceptQr} />
              <div className="cms-actions">
                <button
                  disabled={busy}
                  onClick={() => void perform(loadRegistrations)}
                >
                  Load online registrations
                </button>
                {registrations.length > 0 && (
                  <button onClick={acceptOnline}>
          ...
          {editable && event.status === "completed" && (
            <div className="cms-actions">
              ...
              <button
                ...
                onClick={() => void perform(awardGold)}
              >
                {event.goldAwarded ? "Gold awarded" : "Award Champion–3rd Gold"}
              </button>
            </div>
          )}
          <section className="tournament-cms-section"><div className="tournament-cms-section-head"><span>04 · ROUNDS</span><h3>Pairings and results</h3></div>{event.history.map((round, ri) => (
            <div className="pair-list" key={ri}>
              ...
              {round.map((g, bi) => (
                <article key={bi}>
                  <strong>Board {bi + 1}</strong>
                  <span>
                    {event.players.find((p) => p.id === g.white)?.name} (White)
                    ·{" "}
                    {g.black
                      ? event.players.find((p) => p.id === g.black)?.name +
                        " (Black)"
                      : "BYE"}
                  </span>
                  {g.black &&
                  ri === event.history.length - 1 &&
                  editable &&
                  event.status !== "completed" ? (
                    <label>
                      Result
                      <select
                        value={g.result}
                        onChange={(e) => result(bi, e.target.value as Result)}
                      >
                    ...
                    <span>{g.result || "Awaiting result"}</span>
                  )}
                </article>
              ))}
            </div>
          ...
          {editable && event.status !== "completed" && (
            <div className="cms-actions">
              {event.history.length < event.rounds ? (
                ...
                >
                  Pair round {event.history.length + 1}
                </button>
              ) : (
                <button
          ...
          <div className="cms-actions tournament-cms-exports">
            ...
            <button
              disabled={busy || event.history.at(-1)?.some((g) => !g.result)}
              onClick={() =>
                void perform(async () =>
                  download(
                    "chess-burger-" + event.code + ".trf",
                    exportTrf(event),
                    "text/plain",
                  ),
                )
        ...
        <>
          ...
          {invite && (
            <>
              <h3>{invite.title}</h3>
              <p>
                Host: {invite.hostName} · Code: {invite.code}
              </p>
              <button disabled={busy} onClick={() => void perform(register)}>
                Register online
              </button>
              <label>
                Name for offline registration
                <input
                  maxLength={60}
                  value={joinName}
              ...
              >
                Create my registration QR
              </button>
              {joinQr && (
                <div className="tournament-qr">
                  <QRCodeSVG value={joinQr} size={240} />
                  <span>Show this QR to the host to finish joining.</span>
                  <textarea
                    aria-label="Registration QR text"
                    readOnly
                    value={joinQr}
                  />
                </div>
              ...
              <p className="cms-note">
                Offline joining uses two scans: your phone reads the host
                invitation, then the host reads your registration. Both devices
                must have opened the app online first. Play on a physical board
                or the app’s offline board; the host records results.
              </p>
            </>
          )}
        </>
