...
export async function settle(db:D1Database,id:string){
 ...
 for(const [side,pid,opponentCbr,ownCbr] of [['white',m.white_id,m.black_cbr,m.white_cbr],['black',m.black_id,m.white_cbr,m.black_cbr]] as const){
  ...
  if(won){
   ...
   statements.push(stmt(db,`INSERT OR IGNORE INTO arena_feed(id,user_id,kind,display_name,content,created_at) SELECT ?,user_id,'first_blood',display_name,'earned their first win of the day.',? FROM arena_players WHERE user_id=? AND ${guard} AND NOT EXISTS(SELECT 1 FROM arena_ledger WHERE id=?)`,'first:'+pid+':'+day,now(),pid,id,'first:'+pid+':'+day));
   statements.push(stmt(db,`INSERT OR IGNORE INTO arena_ledger(id,user_id,delta,kind,created_at) SELECT ?,?,0,'first_blood',? WHERE ${guard}`,'first:'+pid+':'+day,pid,now(),id));
  }
...
export async function publicAction(db:D1Database,action:string,params:URLSearchParams){
 ...
 if(action==='feed'){
  ...
  const result=await stmt(db,'SELECT f.*,COALESCE(p.avatar_url,\'\') AS avatar_url,COALESCE(p.cbr,88) AS cbr,(SELECT COUNT(*) FROM arena_hearts h WHERE h.feed_id=f.id) AS heart_count FROM arena_feed f LEFT JOIN arena_players p ON p.user_id=f.user_id ORDER BY f.created_at DESC LIMIT 50').all();
...
export async function privateAction(db:D1Database,profile:PlayerProfile,action:string,input:Record<string,any>){
 ...
 if(action==='offline-result'){
  ...
  const guard='NOT EXISTS(SELECT 1 FROM arena_ledger WHERE id=?)',day=new Date(r.finished_at).toISOString().slice(0,10),first='first:'+id+':'+day;
  ...
  if(won){writes.push(stmt(db,`INSERT OR IGNORE INTO arena_feed(id,user_id,kind,display_name,content,cbr_delta,created_at) SELECT ?,user_id,'win',display_name,'won an offline match.',${delta},? FROM arena_players WHERE user_id=? AND ${guard}`,key+':win',r.finished_at,id,key));writes.push(stmt(db,`INSERT OR IGNORE INTO arena_feed(id,user_id,kind,display_name,content,created_at) SELECT ?,user_id,'first_blood',display_name,'earned their first win of the day.',? FROM arena_players WHERE user_id=? AND ${guard} AND NOT EXISTS(SELECT 1 FROM arena_ledger WHERE id=?)`,first,r.finished_at,id,key,first));writes.push(stmt(db,`INSERT OR IGNORE INTO arena_ledger(id,user_id,delta,kind,created_at) SELECT ?,?,0,'first_blood',? WHERE ${guard}`,first,id,r.finished_at,key));}
 ...
 if(action==='save-announcement'){
  ...
  const edit=String(input.id??''),feedId=edit||uid(),logId=uid();await db.batch([edit?stmt(db,"UPDATE arena_feed SET content=?,image_url=?,expires_at=? WHERE id=? AND kind='announcement'",content,image,expires,edit):stmt(db,"INSERT INTO arena_feed(id,user_id,kind,display_name,content,image_url,expires_at,created_at) VALUES(?,?,'announcement',?,?,?,?,?)",feedId,id,p.display_name,content,image,expires,t),stmt(db,'INSERT INTO arena_logs(id,actor_user_id,action,details,created_at) VALUES(?,?,?,?,?)',logId,id,edit?'announcement_update':'announcement_create',JSON.stringify({id:feedId,expires_at:new Date(expires).toISOString()}),t),stmt(db,'DELETE FROM arena_feed WHERE id NOT IN(SELECT id FROM arena_feed ORDER BY created_at DESC LIMIT 50)')]);return {ok:true};
 }
 ...
 if(action==='logs'){const page=Math.max(0,Math.min(100,Number(input.page)||0));const r=await stmt(db,'SELECT * FROM arena_logs ORDER BY created_at DESC LIMIT 20 OFFSET ?',page*20).all();return {logs:r.results.map(log=>({...log,details:JSON.parse(log.details as string),created_at:new Date(log.created_at as number).toISOString()}))};}
 throw new ArenaError('Unknown action.',404);
}
