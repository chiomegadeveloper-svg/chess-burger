...
export default function Rankings({profile}:{profile:PlayerProfile|null}){
 ...
 return <section className="rank-page"><div className="page-heading"><h1>CBR Rankings</h1><span className="sample-label">Live standings</span></div><div className="leaderboard cloud-leaderboard"><div className="leaderboard-heading"><h2>Top 10</h2><span>CBR / Gold</span></div>{players.map((p,i)=>{const Icon=[Crown,Medal,Trophy][i];return <div className="ranking-line" key={p.user_id}><span className={'rank-number podium-'+i}>{Icon?<Icon aria-label={'Rank '+(i+1)} size={19}/>:String(i+1).padStart(2,'0')}</span><Avatar player={p}/><strong>{p.display_name}</strong><span className="rating">{p.cbr.toLocaleString()}<small>{p.gold_points.toLocaleString()} Gold</small></span></div>})}{!players.length&&<p className="account-note">{error||'Players will appear after signing in to the updated app.'}</p>}</div><div className="current-rank current-rank-bottom"><span>Your rank<strong>{mine?'#'+mine.rank:'Unranked'}</strong></span><span>CBR<strong>{self?.cbr??88}</strong></span><span>Level<strong>{levelFor(self?.cbr??88).level}</strong></span></div>{error&&players.length>0&&<p role="status">{error}</p>}</section>;
}
