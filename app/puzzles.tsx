...
import { ArrowLeft, Bot, Check, Coins, Lightbulb, LockKeyhole, RotateCcw, Sparkles, Volume2 } from "lucide-react";
...
export default function Puzzles({ onClose, onReward }: { onClose: () => void; onReward: () => void }) {
  ...
  return <section className="puzzle-page">
    ...
    <div className="puzzle-workspace">
      <div className="puzzle-board-wrap"><div className="puzzle-board" aria-label={`Chess puzzle: ${puzzle.title}`}>{game.board().flat().map((piece, cell) => { const square = (`${"abcdefgh"[cell % 8]}${8 - Math.floor(cell / 8)}`) as Square; return <button type="button" key={square} aria-label={`${square}${piece ? ` ${piece.color === "w" ? "white" : "black"} ${piece.type}` : " empty"}`} className={`puzzle-square ${(Math.floor(cell / 8) + cell) % 2 ? "dark" : "light"} ${selected === square ? "selected" : ""} ${legal.includes(square) ? "legal" : ""} ${piece?.color === "w" ? "white-piece" : ""}`} onClick={() => void play(square)}><span>{piece ? symbols[piece.color + piece.type] : ""}</span></button>; })}</div></div>
      <aside className="puzzle-coach"><div className="coach-avatar"><Bot/><i><Sparkles/></i></div><span>Free local coach</span><h2>Coach Patty</h2><p>{message}</p>{hintOpen && <div className="coach-hint"><Lightbulb/>{puzzle.hint}</div>}<div className="coach-actions"><button type="button" onClick={() => setHintOpen(value => !value)}><Lightbulb/>Hint</button><button type="button" onClick={coachSpeak}><Volume2/>Read</button><button type="button" onClick={reset}><RotateCcw/>Reset</button></div><div className="puzzle-detail"><span>Level {index + 1} of 10</span><strong>{puzzle.title}</strong><small>{puzzle.theme} · White to move</small></div></aside>
    </div>
  </section>;
}
