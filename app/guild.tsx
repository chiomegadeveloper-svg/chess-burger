...
import {Castle,ChevronLeft,ChevronRight,Crown,ImagePlus,Users,CalendarClock} from 'lucide-react';
...
type State={page:number;total:number;eligible:boolean;my_guild_id:string|null;kick_notice?:{guild_id:string;reason:string;created_at:string;cb_guilds?:{name:string}|null}|null;guilds:Guild[];detail:Guild|null};
...
export default function GuildPage({profile,onChanged}:{profile:PlayerProfile|null;onChanged?:()=>void}){
 ...
 const detail=state?.detail,member=!!detail&&state?.my_guild_id===detail.id,editor=!!detail&&member&&!!profile&&[detail.creator_id,detail.leader_id].includes(profile.user_id),leader=member&&detail.leader_id===profile?.user_id,pages=Math.max(1,Math.ceil((state?.total??0)/10));
 return <section className="guild-page"><div className="guild-heading"><div className="guild-heading-copy"><span>CHESS BURGER GUILDS</span><h1><Castle/> Guild Hall</h1><p>Team up, earn CBG for your guild chest, and climb the Guild Points board.</p></div><button type="button" disabled={busy||!state?.eligible||!!state?.my_guild_id} onClick={()=>{const value=window.prompt('Guild name (3–12 characters). Creating a guild costs 100 CBG.')?.trim();if(value){if(value.length<3||value.length>12)toast.error('Guild name must be 3 to 12 characters.');else if(window.confirm(`Create ${value} for 100 CBG?`))void act('create',{name:value})}}} title={!state?.eligible?'Reach Level 3 (177 CBR) first':state?.my_guild_id?'Leave your guild before creating another':''}>Create Guild · 100 CBG</button></div>
 {!state?.my_guild_id&&state?.kick_notice&&<p className="guild-notice guild-kick-notice">You were removed from {state.kick_notice.cb_guilds?.name||"a guild"}. Reason: {state.kick_notice.reason}</p>}
 ...
 <h3>Members</h3><div className="guild-members">{detail.members?.map(m=><div className="guild-member" key={m.user_id}><img src={m.profile?.avatar_url||'/avatar-default.svg'} alt=""/><span><strong>{m.profile?.display_name||m.profile?.username||'Player'} {detail.leader_id===m.user_id&&<Crown size={15} aria-label="Guild leader"/>}</strong><small>@{m.profile?.username||'player'} · {Number(m.profile?.cbr??0).toLocaleString()} CBR</small></span>{member&&detail.leader_id!==m.user_id&&<button disabled={busy} onClick={()=>void act('vote',{candidate_id:m.user_id})}>Vote leader</button>}{leader&&detail.leader_id!==m.user_id&&<button className="guild-kick" disabled={busy} onClick={()=>{const reason=window.prompt(`Reason for removing ${m.profile?.display_name||"this member"} (5–240 characters):`)?.trim();if(reason){if(reason.length<5||reason.length>240)toast.error("Enter a reason of 5 to 240 characters.");else if(window.confirm(`Remove ${m.profile?.display_name||"this member"} from the guild?`))void act("kick",{member_id:m.user_id,reason})}}}>Kick</button>}</div>)}</div><p className="guild-rule">A candidate becomes leader after receiving more than half of current members' votes. The creator and leader can edit guild images.</p>
 <div className="guild-actions">{!state?.my_guild_id&&<button disabled={busy||!state?.eligible||detail.member_count>=18} onClick={()=>void act('join',{guild_id:detail.id})}>Join Guild</button>}{member&&<button disabled={busy} onClick={()=>{if(window.confirm(detail.member_count===1?'Quit and close this guild? Remaining chest CBG will be paid to you.':'Quit this guild?'))void act('leave')}}>Quit Guild</button>}</div></article>}</div>}</section>;
}
