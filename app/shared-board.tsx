...
export default function SharedBoard({profile,control,onResult}:{profile:PlayerProfile|null;control:string;onResult:(m:ArenaMatch,id:string)=>void}){
 ...
 function move(input:{from:string;to:string;promotion?:string}){if(checkClock())return;const m=current.current;if(!m||m.status!=='active')return;try{const chess=gameFromPgn(m.pgn),white=chess.turn()==='w',t=Date.now();chess.move(input);const delta=t-m.last_tick,inc=timeControl(m.control).increment*1000,result=boardResult(chess);commit({...m,pgn:chess.pgn(),version:m.version+1,white_ms:white?Math.max(0,m.white_ms-delta)+inc:m.white_ms,black_ms:white?m.black_ms:Math.max(0,m.black_ms-delta)+inc,last_tick:t,result,status:result?'finished':'active'});}catch{setError('Choose a legal move.');}}
