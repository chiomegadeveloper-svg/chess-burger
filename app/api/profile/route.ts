...
async function handle(request:Request){
 try{
  ...
  if(!/^https:\/\//i.test(String(input.avatar_url??'').trim()))return reply({error:'A profile picture is required to unlock Chess Burger.',code:'avatar_required'},400);
 ...
 }catch(e){const message=(e as Error).message;if(message==='account_deleted')return reply({error:'This Chess Burger account has been deleted by the owner.',code:message},403);if(message==='username_taken')return reply({error:'That username is already taken. Choose another one.',code:message},409);if(message==='username_format')return reply({error:'Username must use 3–24 lowercase letters, numbers, or underscores.',code:message},400);if(message==='name_required')return reply({error:'Enter your name (up to 60 characters).',code:message},400);if(message==='avatar_required')return reply({error:'A profile picture is required to unlock Chess Burger.',code:message},400);console.error('Profile request failed',e);return reply({error:'Profile could not be saved. Please retry.',code:'profile_save_failed'},500);}
}
