...
// Project owner allowlist. Checked server-side after Supabase authentication so
// the designated account cannot be downgraded by a profile save or import.
const designatedOwners=new Set(['alota.bobbie.2026@gmail.com']);
const isDesignatedOwner=(email:string|undefined)=>!!email&&designatedOwners.has(email.trim().toLowerCase());
...
function profileFromRow(row:ProfileRow):PlayerProfile{return {...row,featured_photos:stringList(row.featured_photos,4),featured_badges:stringList(row.featured_badges,5),role:roles.has(row.role)?row.role as PlayerProfile['role']:'player',created_at:new Date(Number(row.created_at)).toISOString()};}
...
export async function saveProfile(db:D1Database,user:User,input:Partial<PlayerProfile>,options:{trusted?:boolean;role?:PlayerProfile['role'];announce?:boolean}={}){
 ...
 const t=Date.now(),created=existing?Date.parse(existing.created_at):t,bio=String(input.bio??'').trim().slice(0,240),avatar=cleanUrl(input.avatar_url),country=/^[A-Z]{2}$/.test(String(input.country_code??''))?String(input.country_code):'PH';if(!avatar)throw new Error('avatar_required');
 const photos=stringList(input.featured_photos,4).map(cleanUrl),badges=stringList(input.featured_badges,5),role=isDesignatedOwner(user.email)?'owner':existing?.role??options.role??'player',cardPhoto=options.trusted?cleanUrl(input.card_photo_url):existing?.card_photo_url??'';
...
export async function loadOrImportProfile(db:D1Database,user:User,client:SupabaseClient){
 ...
 if(saved){
  if(isDesignatedOwner(user.email)&&saved.role!=='owner'){
   await stmt(db,'UPDATE app_profiles SET role=?,updated_at=? WHERE user_id=?','owner',Date.now(),user.id).run();
   return (await readProfile(db,user.id))!;
  }
  return saved;
 }
