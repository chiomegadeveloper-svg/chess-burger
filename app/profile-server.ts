...
export async function saveProfile(db:D1Database,user:User,input:Partial<PlayerProfile>,options:{trusted?:boolean;role?:PlayerProfile['role'];announce?:boolean}={}){
 ...
 const photos=stringList(input.featured_photos,4).map(cleanUrl),badges=stringList(input.featured_badges,5),role=existing?.role??options.role??'player',cardPhoto=options.trusted?cleanUrl(input.card_photo_url):existing?.card_photo_url??'';
