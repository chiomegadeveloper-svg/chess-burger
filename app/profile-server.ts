...
const profileSelect=`SELECT p.*,COALESCE(a.cbr,88) AS cbr,COALESCE(a.gold_points,0) AS gold_points,COALESCE(a.win_streak,0) AS win_streak,COALESCE(a.wins,0) AS wins,COALESCE(a.losses,0) AS losses FROM app_profiles p LEFT JOIN arena_players a ON a.user_id=p.user_id`;
...
export async function saveProfile(db:D1Database,user:User,input:Partial<PlayerProfile>,options:{trusted?:boolean;role?:PlayerProfile['role'];announce?:boolean}={}){
 ...
 const writes=[
  ...
  stmt(db,`INSERT INTO arena_players(user_id,username,display_name,avatar_url,country_code,cbr,gold_points,wins,losses,win_streak,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(user_id) DO UPDATE SET username=excluded.username,display_name=excluded.display_name,avatar_url=excluded.avatar_url,country_code=excluded.country_code,updated_at=excluded.updated_at`,user.id,username,displayName,avatar,country,cbr,gold,wins,losses,streak,t),
 ];
