...
export class LanConnection {
 pc:RTCPeerConnection;channel:RTCDataChannel|null=null;host:boolean;room:string;own:ArenaPlayer;control:string;match:ArenaMatch|null=null;closed=false;timer:ReturnType<typeof setInterval>|null=null;
 ...
 attach(channel:RTCDataChannel){this.channel=channel;channel.onopen=()=>{this.send({type:'hello',player:this.own});this.onStatus('Connected over local Wi-Fi');};channel.onclose=()=>this.onStatus('Connection closed. Your replay is saved on this device.');channel.onmessage=e=>{try{if(typeof e.data!=='string'||e.data.length>30000)return;this.receive(JSON.parse(e.data));}catch{this.onStatus('An invalid game message was rejected.');}};}
 ...
 receive(message:any){
  if(message.type==='hello'&&this.host&&!this.match){const p=message.player;if(!p||typeof p.user_id!=='string'||p.user_id===this.own.user_id||typeof p.display_name!=='string')return;
   const guest:ArenaPlayer={user_id:p.user_id.slice(0,80),display_name:p.display_name.slice(0,60),username:String(p.username??'player').slice(0,24),avatar_url:'',country_code:'',cbr:Number.isFinite(p.cbr)?Math.max(0,Math.min(100000,p.cbr)):88,gold_points:0,wins:0,losses:0,win_streak:0};
   const t=timeControl(this.control);this.match={id:this.room,host_id:this.own.user_id,white_id:this.own.user_id,black_id:guest.user_id,invite_to:null,code:this.room.slice(0,8),control:this.control,status:'active',pgn:'',white_ms:t.seconds*1000,black_ms:t.seconds*1000,last_tick:Date.now(),version:0,result:null,white_cbr:this.own.cbr,black_cbr:guest.cbr,rating_applied:0,created_at:Date.now(),white:{...this.own,avatar_url:''},black:guest};this.publish();this.timer=setInterval(()=>this.checkClock(),250);
 ...
 apply(move:any,resign:boolean,actor:string,version:number){
  ...
  if(resign)m.result=actor===m.white_id?'black':'white';else {if(actor!==(white?m.white_id:m.black_id))return;try{chess.move({from:move.from,to:move.to,promotion:move.promotion??'q'});}catch{return;}const elapsed=Date.now()-m.last_tick,inc=timeControl(m.control).increment*1000;if(white)m.white_ms=Math.max(0,m.white_ms-elapsed)+inc;else m.black_ms=Math.max(0,m.black_ms-elapsed)+inc;m.pgn=chess.pgn();m.result=boardResult(chess);}
