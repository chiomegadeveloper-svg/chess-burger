...
export default function ClassroomVoice({roomId,role,onRaisedHands,students=[],acknowledgedHand,mediaOpen=false,onMediaOpenChange}:{roomId:string;role:VoiceRole;onRaisedHands?:(ids:string[])=>void;students?:{user_id:string;display_name?:string;username?:string}[];acknowledgedHand?:{id:string;sequence:number}|null;mediaOpen?:boolean;onMediaOpenChange?:(open:boolean)=>void}){
  ...
  return <div className={`classroom-voice ${status}`}>
    ...
    {role==="teacher"&&mediaOpen&&createPortal(<section className="seba-media-controls" role="dialog" aria-labelledby="seba-media-title">
      <header className="seba-media-heading"><div className="seba-media-heading-icon"><img src="/classroom/class-media.webp" alt=""/></div><div><small>TEACHER CONTROLS</small><h2 id="seba-media-title">Class media</h2></div><button type="button" className="seba-media-close" onClick={()=>onMediaOpenChange?.(false)} aria-label="Close class media"><X/></button></header>
      {status!=="live"?<p className="seba-media-notice">Connect to SEba Voice in the top bar to manage student audio and cameras.</p>:<>
        <div className="seba-media-actions"><button type="button" disabled={mediaBusy} onClick={()=>void mediaAction("mute-all")}><MicOff/><span>Mute all students</span></button><button type="button" disabled={mediaBusy} onClick={()=>void mediaAction("close-all-cameras")}><CameraOff/><span>Close all cameras</span></button></div>
        <div className="seba-media-roster"><div className="seba-media-roster-title"><strong>Students</strong><span>{students.length}</span></div>{students.length===0&&<p className="seba-media-empty">Students will appear here when they join.</p>}{students.map(student=><div className="seba-media-student" key={student.user_id}><span className="seba-media-avatar" aria-hidden="true">{(student.display_name||student.username||"S").charAt(0).toUpperCase()}</span><b title={student.display_name||student.username||"Student"}>{student.display_name||student.username||"Student"}</b><div><button type="button" disabled={mediaBusy} onClick={()=>void mediaAction("mute-student",student.user_id)} aria-label={`Mute ${student.display_name||student.username||"student"}`}><MicOff/><span>Mute</span></button><button type="button" disabled={mediaBusy} onClick={()=>void mediaAction("request-unmute",student.user_id)} aria-label={`Ask ${student.display_name||student.username||"student"} to unmute`}><Mic/><span>Ask to unmute</span></button></div></div>)}</div>
        <p className="seba-media-note">Students accept unmute requests themselves. Only a camera owner can turn their camera on.</p>
      </>}
