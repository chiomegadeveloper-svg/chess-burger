  ...
  return <div className={`classroom-voice ${status}`}>
    ...
    {status==="idle"?<div className="voice-permission-actions"><button type="button" className={permission==="denied"?"permission-blocked":""} onClick={()=>permission==="denied"?setShowPermissionHelp(true):void start()} title={permission==="denied"?"Open microphone permission help":`Allow microphone and join SEba Voice as ${role}`}>{permission==="denied"?<MicOff/>:<Mic/>}{permission==="denied"?"Mic blocked":"Enable Microphone"}</button>{permission==="denied"&&<button type="button" className="voice-settings" onClick={()=>setShowPermissionHelp(true)} aria-label="Microphone settings help"><Settings/></button>}</div>:
      status==="starting"?<button type="button" disabled><Radio/>Connecting…</button>:<>
        ...
        {needsAudioStart&&<button type="button" className="enable-sound" onClick={()=>void enableSound()} title="Enable classroom sound"><Volume2/>Enable Sound</button>}
        <button type="button" className={muted?"muted":""} onClick={()=>void toggleMute()} title={muted?"Unmute microphone":"Mute microphone"}>{muted?<MicOff/>:<Mic/>}{muted?"Unmute":"Mute"}</button>
