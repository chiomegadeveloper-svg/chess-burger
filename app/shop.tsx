...
import { ArrowLeft, Check, Crown, Gift, PackageOpen, ShoppingBag, Ticket } from "lucide-react";
...
function ArenaTicketStore({fallbackGold}:{fallbackGold:number}){
  ...
  return <section className="arena-ticket-store"><div className="arena-ticket-store-heading"><div className="ticket-art"><img src="/grand-arena/arena-ticket.webp" alt="Chess Burger Arena Ticket"/></div><div><span>GRAND ARENA ACCESS</span><h2>Arena Tickets</h2><p>One ticket admits one competitive Arena run.</p></div><b><Ticket size={14}/>{tickets} owned</b></div><div className="arena-ticket-bundles">{ARENA_BUNDLES.map((bundle,index)=><article key={bundle.quantity} className={index===1?'featured':''}><div className="ticket-product-icon"><Ticket size={18}/><b>×{bundle.quantity}</b></div><div className="ticket-product-copy"><strong>{bundle.name}</strong><span>{bundle.quantity} ticket{bundle.quantity>1?'s':''}</span><small>{bundle.quantity>1?`${Math.round((1-bundle.price/(bundle.quantity*28))*100)}% bundle savings`:'Standard admission'}</small></div><button type="button" disabled={busy!==null||balance<bundle.price} onClick={()=>void buy(bundle)}>{busy===bundle.quantity?'Purchasing…':<><i className="shop-gold-dot"/>{bundle.price} Gold</>}</button></article>)}</div></section>;
}
...
export function ShopPage({ profile, onChanged }: { profile: PlayerProfile | null; onChanged: () => void }) {
  ...
  return <section className="shop-page shop-home">
    <div className="page-heading shop-main-heading"><div><span className="shop-eyebrow">CHESS BURGER MARKET</span><h1>Shop</h1><p>Upgrade your look and unlock competitive experiences.</p></div><span className="banner-wallet"><span className="gold-coin">●</span><strong>{state.gold || profile?.gold_points || 0}</strong><small>Gold balance</small></span></div>
...
export function BagPage({ onChanged }: { onChanged: () => void }) {
  type BagState={banners:BannerRental[];tickets:number;items:Array<{item_kind:string;item_id:string;quantity:number;metadata?:{name?:string}}>;active:string;gold:number};
  const[state,setState]=useState<BagState>({banners:[],tickets:0,items:[],active:"",gold:0}),[loading,setLoading]=useState(true),[busy,setBusy]=useState(""),[giftItem,setGiftItem]=useState<{kind:string;id:string;name:string;max:number}|null>(null),[username,setUsername]=useState(""),[quantity,setQuantity]=useState(1);
  const load=async()=>{const data=await arena<BagState>("bag-items");setState(data);};
  useEffect(()=>{void load().catch(error=>toast.error(error.message)).finally(()=>setLoading(false));},[]);
  const activate=async(id:string)=>{setBusy(id);try{const data=await arena("activate-feed-banner",{product_id:id}) as {active:string};setState(current=>({...current,active:data.active}));toast.success(`${feedBanner(id)?.name??"Banner"} activated.`);onChanged();}catch(error){toast.error(error instanceof Error?error.message:"Unable to activate item.");}finally{setBusy("");}};
  const openGift=(kind:string,id:string,name:string,max=1)=>{setGiftItem({kind,id,name,max});setUsername("");setQuantity(1);};
  const sendGift=async()=>{if(!giftItem||busy)return;setBusy(`gift:${giftItem.kind}:${giftItem.id}`);try{await arena("gift-bag-item",{username,item_kind:giftItem.kind,item_id:giftItem.id,quantity,request_id:crypto.randomUUID()});toast.success(`${giftItem.name} sent to ${username}. 4 Gold charged.`);setGiftItem(null);await load();onChanged();}catch(error){toast.error(error instanceof Error?error.message:"Unable to send gift.");}finally{setBusy("");}};
  const count=state.banners.length+state.items.length+(state.tickets>0?1:0);
  return <section className="bag-page"><div className="page-heading"><div><h1>My Bag</h1><p>Tickets, banners, and future collectibles appear here. Every gift transaction costs 4 Gold.</p></div><span className="bag-count"><ShoppingBag size={16}/>{count}</span></div>
    {loading?<p className="account-note">Opening your bag…</p>:count===0?<div className="empty-bag"><PackageOpen size={42}/><h2>Your bag is empty</h2><p>Purchased Arena Tickets and Feed Banners will appear here.</p></div>:<div className="bag-tiles unified-bag-grid">
      {state.tickets>0&&<article className="bag-inventory-card ticket"><img src="/grand-arena/arena-ticket.webp" alt="Arena Ticket"/><div><small>GRAND ARENA</small><h3>Arena Ticket</h3><strong>×{state.tickets}</strong></div><button type="button" onClick={()=>openGift('arena_ticket','arena-ticket','Arena Ticket',state.tickets)}><Gift size={14}/>Gift</button></article>}
      {state.banners.map(rental=>{const banner=feedBanner(rental.product_id);if(!banner)return null;const active=state.active===rental.product_id;return <article className={`bag-inventory-card banner ${active?'active':''}`} key={rental.product_id}><span className="banner-swatch" style={{background:banner.background}}/><div><small>{banner.tier} banner</small><h3>{banner.name}</h3><span>{rentalLabel(rental.expires_at)}</span></div><div><button type="button" disabled={busy===rental.product_id||active} onClick={()=>void activate(rental.product_id)}>{active ? <><Check size={13}/> Active</> : "Use"}</button><button type="button" onClick={()=>openGift('feed_banner',rental.product_id,`${banner.name} Banner`)}><Gift size={14}/>Gift</button></div></article>;})}
      {state.items.map(item=><article className="bag-inventory-card generic" key={`${item.item_kind}:${item.item_id}`}><PackageOpen/><div><small>{item.item_kind.replaceAll('_',' ')}</small><h3>{item.metadata?.name??item.item_id.replaceAll('-',' ')}</h3><strong>×{item.quantity}</strong></div><button type="button" onClick={()=>openGift(item.item_kind,item.item_id,item.metadata?.name??item.item_id,item.quantity)}><Gift size={14}/>Gift</button></article>)}
    </div>}
    {giftItem&&<div className="bag-gift-backdrop" role="presentation" onClick={()=>setGiftItem(null)}><form className="bag-gift-dialog" role="dialog" aria-modal="true" aria-labelledby="bag-gift-title" onClick={event=>event.stopPropagation()} onSubmit={event=>{event.preventDefault();void sendGift();}}><Gift/><div><small>4 GOLD GIFT FEE</small><h2 id="bag-gift-title">Gift {giftItem.name}</h2></div><label>Recipient username<input autoFocus required value={username} onChange={event=>setUsername(event.target.value)} placeholder="@username"/></label>{giftItem.max>1&&<label>Quantity<input required type="number" min={1} max={giftItem.max} value={quantity} onChange={event=>setQuantity(Math.max(1,Math.min(giftItem.max,Number(event.target.value))))}/></label>}<p>The item transfers directly to the recipient’s Bag. Your account is charged exactly 4 Gold for this transaction.</p><div><button type="button" onClick={()=>setGiftItem(null)}>Cancel</button><button className="gold-button" disabled={!!busy||!username.trim()}>Send gift</button></div></form></div>}
  </section>;
}
