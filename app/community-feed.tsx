 ...
 return <section className="feed-page">
  ...
  {tab==='online'&&<section className="online-directory" aria-label="Online players">
   ...
   {visibleOnline.length>0&&<div className="online-avatars" style={{display:"grid",gridTemplateColumns:"repeat(6,minmax(0,1fr))",alignItems:"start",justifyItems:"center",gap:"18px 10px",width:"100%",maxWidth:"720px",margin:"0 auto"}}>{visibleOnline.map(player=>{const open=onlineCard===player.user_id,own=player.user_id===userId;return <div className="online-player" style={{position:"relative",display:"grid",placeItems:"center",width:"100%",minWidth:0}} key={player.user_id} onMouseEnter={()=>setOnlineCard(player.user_id)} onMouseLeave={()=>setOnlineCard(null)}>
    ...
    {open&&<article className="online-user-card" role="dialog" aria-label={`${player.display_name} player card`} style={{position:"absolute",zIndex:80,top:"calc(100% + 9px)",left:"50%",width:"220px",maxWidth:"min(220px,calc(100vw - 24px))",padding:"12px",transform:"translateX(-50%)",overflow:"visible",boxSizing:"border-box",border:"1px solid #596470",borderRadius:"13px",background:"linear-gradient(150deg,#2b333d,#171d23)",boxShadow:"0 16px 36px #000c"}}><button className="online-card-close" type="button" onClick={()=>setOnlineCard(null)} aria-label="Close player card"><X size={15}/></button><button type="button" className="online-card-identity" style={{display:"grid",gridTemplateColumns:"38px minmax(0,1fr)",gridTemplateRows:"auto auto",columnGap:"9px",alignItems:"center",width:"100%",padding:"2px 22px 10px 0",border:0,background:"transparent",color:"inherit",textAlign:"left"}} onClick={()=>onOpenProfile(player.user_id)}><span style={{gridRow:"1 / 3",display:"grid",placeItems:"center",width:"38px",height:"38px",border:"1px solid #d5ad54",borderRadius:"50%",overflow:"hidden",background:"#1d7885"}}>{player.avatar_url?<img src={player.avatar_url} alt="" style={{display:"block",width:"100%",height:"100%",maxWidth:"38px",maxHeight:"38px",objectFit:"cover",objectPosition:"center",borderRadius:"50%"}}/>:player.display_name.charAt(0)}</span><strong style={{overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",fontSize:"12px"}}>{player.display_name}</strong><small style={{overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",color:"#98dbe1",fontSize:"9px"}}>@{player.username} · {player.cbr} CBR</small></button>{own?<p>This is you.</p>:player.available?<button className="gold-button" type="button" onClick={()=>onChallenge(player)}><Swords size={15}/> Challenge</button>:<p>This player is in a match.</p>}<button className="online-cancel" type="button" onClick={()=>setOnlineCard(null)}>Cancel</button></article>}
   </div>})}</div>}
