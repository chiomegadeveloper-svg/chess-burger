...
export default function CommunityFeed({onOpenProfile,onMatch,onChallenge}:{onOpenProfile:(userId:string)=>void;onMatch:(id:string)=>void;onChallenge:(player:ArenaPlayer)=>void}){
 ...
 return <section className="feed-page">
  ...
  {tab==='online'&&<section className="online-directory" aria-label="Online players">
   ...
   {visibleOnline.length>0&&<div className="online-avatars">{visibleOnline.map(player=>{const open=onlineCard===player.user_id,own=player.user_id===userId;return <div className="online-player" key={player.user_id} onMouseEnter={()=>setOnlineCard(player.user_id)} onMouseLeave={()=>setOnlineCard(null)}>
    <button className="online-avatar-button" type="button" onClick={()=>setOnlineCard(current=>current===player.user_id?null:player.user_id)} aria-expanded={open} aria-label={`Open ${player.display_name}'s player card`}><span className="online-avatar" style={{display:"grid",placeItems:"center",width:"clamp(38px,5vw,52px)",height:"clamp(38px,5vw,52px)",aspectRatio:"1",borderRadius:"50%",overflow:"hidden",flex:"0 0 auto"}}>{player.avatar_url?<img src={player.avatar_url} alt="" style={{display:"block",width:"100%",height:"100%",maxWidth:"100%",objectFit:"cover",objectPosition:"center",borderRadius:"50%"}}/>:player.display_name.charAt(0)}</span></button>
