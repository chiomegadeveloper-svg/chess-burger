...
export default function CommunityFeed({onOpenProfile}:{onOpenProfile:(userId:string)=>void}){
 ...
 return <section className="feed-page">
  ...
  <ol className="community-list">{events.map(event=>{const date=new Date(event.created_at),level=levelFor(event.cbr),announcement=event.kind==='announcement';return <li key={event.id} className={`feed-cloud kind-${event.kind}`}>
   <button className="feed-player" disabled={announcement} onClick={()=>!announcement&&onOpenProfile(event.user_id)} aria-label={announcement?'Chess Burger announcement':`Open ${event.display_name}'s profile`}><span className="feed-avatar">{event.avatar_url?<img src={event.avatar_url} alt=""/>:event.display_name.charAt(0)}</span>{!announcement&&<img className="feed-level" src={`/levels/level-${String(level.level-1).padStart(2,"0")}.png`} alt={`Level ${level.level}`}/>}</button>
