...
export default function CommunityFeed({onOpenProfile,onMatch,onChallenge,onArena}:{onOpenProfile:(userId:string)=>void;onMatch:(id:string)=>void;onChallenge:(player:ArenaPlayer)=>void;onArena:()=>void}){
 ...
 return <section className="feed-page">
  ...
  <ol className="community-list">{events.map(event=>{const date=new Date(event.created_at),level=levelFor(event.cbr),announcement=event.kind==='announcement',avatarUrl=announcement?'/cburger_logo.png':event.avatar_url,banner=feedBanner(event.feed_banner),bannerStyle=banner?{"--feed-banner":banner.background,"--feed-banner-ink":banner.ink} as CSSProperties:undefined;return <li key={event.id} className={`feed-cloud kind-${event.kind}${banner?" has-feed-banner":""}`} style={bannerStyle}>
