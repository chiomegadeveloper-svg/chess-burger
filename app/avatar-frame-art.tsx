...
export function AvatarFrameOverlay({ frameId }: { frameId?: string | null }) {
  ...
  return <span className={`avatar-frame-overlay avatar-frame-${frame.tier}`} data-frame-row={Math.floor(frame.index / 5)} style={{ backgroundPosition: `${(frame.index % 5) * 25}% ${Math.floor(frame.index / 5) * 100}%` }} aria-hidden="true" />;
}
