...
const puzzleDay=()=>{const parts=new Intl.DateTimeFormat('en-US',{timeZone:'Asia/Manila',year:'numeric',month:'2-digit',day:'2-digit'}).formatToParts(new Date(Date.now()-60_000)),part=(type:string)=>parts.find(item=>item.type===type)?.value??'';return `${part('year')}-${part('month')}-${part('day')}`;};
const dailyPuzzleTracks=(day:string)=>{const ordinal=Math.floor(Date.parse(`${day}T00:00:00Z`)/86_400_000);let seed=(ordinal*1664525+1013904223)>>>0;const shuffle=(ids:string[])=>{const next=[...ids];for(let index=next.length-1;index>0;index--){seed=(seed*1664525+1013904223)>>>0;const swap=seed%(index+1);[next[index],next[swap]]=[next[swap]!,next[index]!]}return next};const pick=(ids:string[],count:number)=>Array.from({length:count},(_,index)=>ids[(ordinal*count+index)%ids.length]!);const original=DAILY_PUZZLES.map(item=>item.id),easy=shuffle(original.slice(0,30)),regular=shuffle(original.slice(30,70)),hard=shuffle(original.slice(70)),random=shuffle([...pick(VERY_HARD_IDS,300),...pick(OTHER_IDS,200)]);return {easy:easy.slice(0,5),regular:regular.slice(0,5),hard:hard.slice(0,5),random};};
const puzzleRating=(id:string)=>PUZZLE_BANK_PROOFS[id]?.rating??(DAILY_PUZZLES.findIndex(item=>item.id===id)<30?900:DAILY_PUZZLES.findIndex(item=>item.id===id)<70?1500:2100);
const puzzleReward=(id:string)=>PUZZLE_BANK_PROOFS[id]?(PUZZLE_BANK_PROOFS[id].rating>=2200?3:2):(DAILY_PUZZLES.findIndex(item=>item.id===id)<30?1:DAILY_PUZZLES.findIndex(item=>item.id===id)<70?2:3);
...
export default async function handler(req: Req, res: Res) {
  ...
  try {
    ...
    if(action==='claim-puzzle'){
      ...
      const existing=await client.from('cb_daily_puzzle_claims').select('puzzle_id').eq('user_id',account.id).eq('puzzle_day',day);
    ...
    if(action==='buy-feed-banner'){
      ...
      if(!/^(pastel|metal|neon)-[a-z0-9-]{1,30}$/.test(productId)||![3,5,7].includes(days)||!/^[a-f0-9]{8}(-[a-f0-9]{4}){3}-[a-f0-9]{12}$/i.test(requestId))fail(400,'Choose a valid Feed Banner rental.');
      ...
      if(bought.error){const message=String(bought.error.message??'Purchase failed.');if(/cb_buy_feed_banner_timed|cb_user_items|expires_at|schema cache|function/i.test(message))fail(503,'Run supabase/0062_color_banner_collections.sql in Supabase, then try again.');if(/not enough gold/i.test(message))fail(409,'You do not have enough Gold for this rental.');fail(409,message);}
      return res.status(200).json(bought.data);
    }
    if(action==='activate-feed-banner'){
      const productId=String(body.product_id??'');if(!/^(pastel|metal|neon|cute|warrior|animated|robot)-[a-z0-9-]{1,30}$/.test(productId))fail(400,'Choose a valid Feed Banner.');
