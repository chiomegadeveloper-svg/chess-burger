...
const dailyPuzzleIds=(day:string)=>{const seed=day.replace(/-/g,'').split('').reduce((total,value,index)=>total+Number(value)*(index+3),0),pick=(start:number,size:number,offset:number)=>DAILY_PUZZLES[start+(seed*offset+offset*7)%size].id,ids=[pick(0,30,3),pick(30,40,5),pick(70,30,7)],randomStart=(seed*11+17)%DAILY_PUZZLES.length;let random=DAILY_PUZZLES[randomStart].id;for(let step=1;ids.includes(random)&&step<DAILY_PUZZLES.length;step++)random=DAILY_PUZZLES[(randomStart+step)%DAILY_PUZZLES.length].id;return [...ids,random];};
...
export default async function handler(req: Req, res: Res) {
  ...
  try {
    ...
    if(action==='puzzle-state'){
      ...
      return res.status(200).json({completed,gold:Number(account.profile.gold_points??0),bonus_claimed:completed.length===4,day,daily_puzzle_ids,...stats.data});
    }
    if(action==='claim-puzzle'){
      ...
      return res.status(200).json({...result,completed:allCompleted,bonus_claimed:allCompleted.length===4,day,daily_puzzle_ids});
    }
