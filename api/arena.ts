...
const dailyPuzzleIds=(day:string)=>{const seed=day.replace(/-/g,'').split('').reduce((total,value,index)=>total+Number(value)*(index+3),0),pick=(start:number,size:number,offset:number)=>DAILY_PUZZLES[start+(seed*offset+offset*7)%size].id;return [pick(0,30,3),pick(30,40,5),pick(70,30,7)];};
...
export default async function handler(req: Req, res: Res) {
  ...
  try {
    ...
    if(action==='puzzle-state'){
      ...
      return res.status(200).json({completed,gold:Number(account.profile.gold_points??0),bonus_claimed:completed.length===3,day,daily_puzzle_ids,...stats.data});
    }
    if(action==='claim-puzzle'){
      ...
      const claimed=await client.rpc('cb_claim_daily_puzzle',{p_user_id:account.id,p_day:day,p_puzzle_id:puzzleId,p_is_final:true,p_puzzle_rating:puzzleRating(puzzleId)});
      ...
      return res.status(200).json({...result,completed:allCompleted,bonus_claimed:allCompleted.length===3,day,daily_puzzle_ids});
    }
