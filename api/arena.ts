...
const dailyPuzzleTracks=(day:string)=>{let seed=day.replace(/-/g,'').split('').reduce((total,value,index)=>total+Number(value)*(index+3),0);const shuffle=(ids:string[])=>{const next=[...ids];for(let index=next.length-1;index>0;index--){seed=(seed*1664525+1013904223)>>>0;const swap=seed%(index+1),current=next[index]!;next[index]=next[swap]!;next[swap]=current}return next};const easy=shuffle(DAILY_PUZZLES.slice(0,30).map(item=>item.id)),regular=shuffle(DAILY_PUZZLES.slice(30,70).map(item=>item.id)),hard=shuffle(DAILY_PUZZLES.slice(70).map(item=>item.id)),random=shuffle(DAILY_PUZZLES.map(item=>item.id));return {easy:easy.slice(0,5),regular:regular.slice(0,5),hard:hard.slice(0,5),random}};
...
export default async function handler(req: Req, res: Res) {
  ...
  try {
    ...
    if(action==='puzzle-state'){
      ...
      return res.status(200).json({completed,gold:Number(account.profile.gold_points??0),bonus_claimed:completed.length===100,day,daily_puzzle_ids,daily_tracks,leaders:leaders.data,...stats.data});
    }
    if(action==='claim-puzzle'){
      ...
      return res.status(200).json({...result,completed:allCompleted,bonus_claimed:allCompleted.length===100,day,daily_puzzle_ids,daily_tracks,leaders:leaders.data});
    }
