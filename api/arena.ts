...
const puzzleDay=()=>{const parts=new Intl.DateTimeFormat('en-US',{timeZone:'Asia/Manila',year:'numeric',month:'2-digit',day:'2-digit'}).formatToParts(new Date()),part=(type:string)=>parts.find(item=>item.type===type)?.value??'';return `${part('year')}-${part('month')}-${part('day')}`;};
const dailyPuzzleIds=(day:string)=>{let seed=day.replace(/-/g,'').split('').reduce((total,value,index)=>total+Number(value)*(index+3),0);const shuffle=(ids:string[])=>{const next=[...ids];for(let index=next.length-1;index>0;index--){seed=(seed*1664525+1013904223)>>>0;const swap=seed%(index+1),current=next[index]!;next[index]=next[swap]!;next[swap]=current}return next},easy=shuffle(DAILY_PUZZLES.slice(0,30).map(item=>item.id)),regular=shuffle(DAILY_PUZZLES.slice(30,70).map(item=>item.id)),hard=shuffle(DAILY_PUZZLES.slice(70).map(item=>item.id)),fixed=[...easy.slice(0,25),...regular.slice(0,25),...hard.slice(0,25)],random=shuffle([...easy.slice(25),...regular.slice(25),...hard.slice(25)]);return [...fixed,...random];};
const puzzleRating=(id:string)=>{const index=DAILY_PUZZLES.findIndex(item=>item.id===id);return index<30?900:index<70?1500:2100;};
const puzzleReward=(id:string)=>{const index=DAILY_PUZZLES.findIndex(item=>item.id===id);return index<30?1:index<70?2:3;};
...
export default async function handler(req: Req, res: Res) {
  ...
  try {
    ...
    if(action==='puzzle-state'){
      const day=puzzleDay(),daily_puzzle_ids=dailyPuzzleIds(day);
      ...
      const [claims,stats]=await Promise.all([client.from('cb_daily_puzzle_claims').select('puzzle_id').eq('user_id',account.id).eq('puzzle_day',day),client.from('cb_puzzle_profiles').select('puzzle_rating,solved_total,correct_streak,best_streak').eq('user_id',account.id).single()]);
      ...
      const completed=(claims.data??[]).map((row:any)=>String(row.puzzle_id)).filter((id:string)=>daily_puzzle_ids.includes(id));
      return res.status(200).json({completed,gold:Number(account.profile.gold_points??0),bonus_claimed:completed.length===100,day,daily_puzzle_ids,...stats.data});
    }
    if(action==='claim-puzzle'){
      ...
      const day=puzzleDay(),daily_puzzle_ids=dailyPuzzleIds(day);
      if(!daily_puzzle_ids.includes(puzzleId))fail(409,"This puzzle is not in today's quest. Refresh Puzzle Quest.");
      ...
      return res.status(200).json({...result,completed:allCompleted,bonus_claimed:allCompleted.length===100,day,daily_puzzle_ids});
    }
