...
The Owner CMS Daily Login editor uses `supabase/0035_editable_daily_login_rewards.sql`. Run this migration in the Supabase project configured by the deployment if it has not already been applied. The seven rewards can be set to Gold, a Feed banner rental, Arena tickets, or Bag slots.
